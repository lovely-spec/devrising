 ...
 ionViewDidEnter() {
    ...
    this.route.queryParams.subscribe(params => {
      if (params && params.special) {
        ...
        this.Pldata = JSON.parse(params.special)
        console.log('ol',this.Pldata);
        this.amount = parseInt(this.Pldata.current_debt).toLocaleString('en-IN') //emi_amount
      }
    })
  }
  ...
  AddPayment(){
    ...
    this.data = {
      ...
      'amount': parseInt(num),
      'slug': this.Pldata.slug,
      'type': 'PL',
      'base_amount': this.Pldata.emi_amount
      
    }
    var num = this.amount;
    var num2 = parseInt(num);
    if(this.Saving['0'].balance_available > num2){
      if(num2 >= parseInt(this.Pldata.emi_amount))
        ...
        this.provider.show_alert('Amount should be greater then '+ parseInt(this.Pldata.emi_amount) +' ')
      }
    }else{
      this.provider.show_alert('Insufficiant balance in your Saving account!')
    }
    // this.provider.AddPayment('RD',this.Pldata.slug,this.amount,this.Saving['0'].account_number).subscribe(data=>{
    //       console.log(data);
    // });
  }
